/**
 * @file video.cpp
 * @brief Cutscene (intro) video playback via software MPEG-4/AAC decode
 *        (FFmpeg), threaded, presented at a fixed 30 fps like the two
 *        reference ports (Prince of Persia, Dungeon-Hunter-2-vita).
 *
 ...
 * path for MPEG-4 Part 2, which is what `ux0:data/asphalt6/data/intro.mp4`
 * actually is (`mp4v` fourcc, MPEG-4 Part 2 Simple Profile, 854x480 @
 * 29.97fps, confirmed via `ffprobe`). Adapted 1:1 from Asphalt-5-Vita's
 * `source/video.cpp` (same Gameloft engine, same MPEG-4 Part 2 assets, same
 * requirement: the port must play the game's original data files unmodified,
 * not a re-encoded copy nobody else could reproduce from their own APK).
 *
...
/*
 * Asphalt 6, unlike Asphalt 5, renders directly at the real 960x544 Vita
 * screen resolution -- gl_init() calls vglInitExtended(0, 960, 544, ...)
 * with no intermediate downsample FBO (see source/utils/glutil.c), and
 * gl_swap() is a plain vitaGL buffer swap, not an upscale-blit. So the
 * video's target rect is just the real screen; intro.mp4 (854x480, 5:3-ish)
 * gets letterboxed by the aspect fit below onto the 960x544 (16:9) screen.
 */
#define VIDEO_TARGET_W 960
#define VIDEO_TARGET_H 544
 ...
 *
 * That GLES2 version caused a real, confirmed-on-hardware regression in
 * Asphalt-5-Vita: the first time it ever ran, every fixed-function draw for
 * the rest of the program's life -- the loading screen, the title screen --
 * came out solid white. `libasphalt5.so` is GLES1.1 fixed-function only and
 * runs through vitaGL's OWN internally-managed shader machinery for that,
 * layered UNDER the same `glUseProgram`/vertex-attribute API surface real
 * custom shaders use -- so introducing a real custom program corrupted
 * state that machinery relies on.
 *
 * Asphalt 6's own engine is GLES2/GLSL-only (compiles shaders at runtime,
 * no fixed-function path at all -- see CLAUDE.md), the reverse situation
 * from Asphalt 5, so this specific regression may not even apply here. But
 * there is no confirmed-on-this-hardware reason yet to risk it: this draw
 * call already saves/restores every piece of GL state it touches (blend,
 * depth test, scissor, cull, texture binding/env mode, viewport, client
 * arrays) around a handful of frames total (a one-off intro, not a
 * per-frame hot path), so the CPU cost of plain fixed-function texturing
 * here is negligible either way. Keep it on this proven-safe path unless a
 * real console test shows a reason not to. (Dungeon-Hunter-2-vita's
 * video.cpp does convert YUV on the GPU with a GLES2 program and two
 * LUMINANCE/LUMINANCE_ALPHA textures instead, which is cheaper on CPU; it
 * is the one thing from that port deliberately not adopted here, for the
...
/*
 * Resolves `name` (a bare filename, e.g. "intro.mp4" -- the argument
 * `nativeLoadMovie()` passes to `GLMediaPlayer.loadMovie()`, which
 * `GLMediaPlayer_loadMovie()` in java.c forwards here verbatim) against a
 * few candidate locations. `DATA_PATH "data/%s"` is the one that actually
 * matches this port's asset layout (`ux0:data/asphalt6/data/intro.mp4`);
 * the rest are kept for robustness in case a future asset lives elsewhere,
 * inherited as-is from Asphalt-5-Vita's version of this function.
 */
    ...
    // ---- video decoder, single-threaded (see VIDEO_DECODE_THREADS) ----
    {
        ...
        if (!codec) {
            ...
            if (P.vctx) {
                ...
                /*
                 * Decode at half resolution -- inherited from Asphalt-5-Vita's
                 * Bug #25 (log #069) fix, same CPU-bound tradeoff applies
                 * here: that port's startup benchmark isolated
                 * yuv420p_planar_to_rgb565() at ~90ms for one 800x480 frame
                 * with no threads/GL/libavcodec involved -- the cost is
                 * intrinsic to pushing that many pixels through this CPU, not
                 * scheduler contention (VIDEO_DECODE_THREADS=1 already ruled
                 * that out there). For Asphalt 6's own intro.mp4 (854x480),
                 * `lowres=1` decodes at 427x240, quartering every
                 * pixel-proportional stage at once: the NEON conversion, the
                 * glTexSubImage2D upload, and the MPEG-4 decode itself. The
                 * draw path upscales with GL_LINEAR onto the real 960x544
                 * screen either way, so no other change is needed. A
                 * skippable cutscene at 30fps soft beats a sharp one at
                 * 10fps -- not yet re-benchmarked on this port's own
                 * hardware, but the tradeoff was decisively confirmed on the
                 * same CPU/toolchain for Asphalt 5.
                 *
